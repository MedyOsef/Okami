...
layout = [[sg.Menu(menu_def)],
          [sg.Text("Interface:", font=('Helvetica Bold', 10)), sg.Combo(values=ifaces, readonly=True, key='-COMBO-', enable_events=True, default_value=ifaces[0])],
          ...
          [sg.Multiline(key="-pktsall-", size=(100, 20), autoscroll=True, disabled=True, background_color='black', text_color='green')],
          [sg.Listbox(values=[], key="-pktslist-", visible=False, enable_events=True, size=(100, 5))]  # Listbox invisible pour la sélection
          ]
...
def pkt_process(pkt):
    ...
    # Déterminer la couleur du texte en fonction du type de paquet
    ...
    # Ajouter le résumé avec la couleur dans le Multiline
    window['-pktsall-'].print(pkt_summary, text_color=color)

    # Ajouter le résumé et le paquet dans les listes correspondantes
    pktsummarylist.append(pkt_summary)
    pkt_list.append(pkt)

    # Mise à jour de la Listbox invisible pour la sélection
    window['-pktslist-'].update(values=pktsummarylist)
...
while True:
    ...
    if event == "-pktslist-" and values["-pktslist-"]:
        selected_packet_index = pktsummarylist.index(values["-pktslist-"][0])
